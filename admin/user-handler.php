...
class UserHandler extends WordPressHooks {
  ...
  public function filter__wp_dropdown_users_args( $query_args, $parsed_args ) {
     ...
     * suppress most of the work for the second run */
    if ( $this->dropdownQueryCache ) {
      $fixed_args ['number'] = 1;
    } else {
  ...
  private function getHtmlSelectInfo( $html ) {
    ...
    foreach ( $selects as $select ) {
      ...
      $fieldName  = $attributes->getNamedItem( 'name' )->nodeValue;
      $className  = $attributes->getNamedItem( 'class' )->nodeValue;
      $options    = $select->getElementsByTagName( 'option' );
      foreach ( $options as $option ) {
        $id          = intval( $option->attributes->getNamedItem( 'value' )->nodeValue );
        $label       = $option->textContent;
        $userList [] = (object) [ 'id' => $id, 'label' => $label ];
      }
    }
    return [ $userList, $fieldName, $className ];
  }
  ...
  public function filter__wp_dropdown_users( $html ) {
    ...
    $result                   = $this->getHtmlSelectInfo( $html );
    $userList                 = $result[0];
    $fieldName                = $result[1];
    $listId                   = $fieldName . '_datalist_' . ( $this->codeGenSerialNo ++ );
    $className                = $result[2];
    $this->dropdownQueryCache = $this->dropdownQueryCache ? $this->dropdownQueryCache : $userList;
    $sourceScript             = '';
    $tag                      = '';
    $sourceScript             .= 'jQuery( document ).ready(function() {' . PHP_EOL;
    $sourceScript             .= "const $listId = " . wp_json_encode( $userList, JSON_PRETTY_PRINT ) . ';' . PHP_EOL;
    $sourceScript             .= "
